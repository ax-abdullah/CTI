...
- **Same normalized vocabulary.** The `AriConnection` translates Stasis events (`StasisStart` → `call.ringing`, `ChannelStateChange` Up → `call.answered`, `StasisEnd`/`ChannelDestroyed` → `call.ended`) into the exact `call.*` events every downstream consumer already handles — so webhooks/CRMs/metrics work unchanged for ARI-driven calls. A `driver='ari'` connection is served by one instance, so it is the sole event source for its calls (no AMI double-emit). *(Correction, Phase 12: when this was written nothing **enforced** that — every replica connected to every ARI row, and each one independently answered channels, played prompts and called `continueInDialplan` on the same live channel. Redis leases are what make the claim true; see [ADR-0012](./0012-single-writer-ownership-for-horizontal-scale.md).)*
...
- Coaching works on plain AMI PBXs (no ARI required); ARI unlocks the finer snoop control when present. Queue stats and IVR routing logic are unit-tested (92 tests total).
- **Verification boundary (honest, updated 2026-07-26):** the routing decision, coaching option mapping, and `AriClient` (against a mock ARI REST+WS) are unit/integration-tested; the coaching Originate executes live against the lab. **The queue wallboard is now verified live**: a call into a configured queue moved `GET /v1/queues` to `waiting:1`, then `abandoned:1` on hangup. Note this path is pure **AMI** — queue stats are aggregated from `app_queue` events and never needed ARI or Stasis, a conflation earlier drafts of this ADR made. It does need `agent` on the manager user's **read** perms (ADR-0001), without which the wallboard is silently empty. Still unverified live: *coached audio* (needs registered SIP phones) and the *ARI/Stasis path itself* (needs `http.conf`/`ari.conf` + a Stasis dialplan, which the lab still lacks). Operator prerequisites, documented in INSTALL — not code gaps.
